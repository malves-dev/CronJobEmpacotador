...
CrontabManager.prototype.add = function (key, tab, task, options) {
  if ((typeof tab === 'string' || tab instanceof Date) && typeof key === 'string' && task instanceof Function) {
    ...
    try {
      ...
      const err = `crontab: ${tab} possibly not valid, job ${key} not started...${fooBaredByUser.message}`;
      console.error(err);
      throw new Error(err);
    }
  } else {
...
CrontabManager.prototype.update = function () {
  if (arguments.length === 2) {
    ...
    const err = `incorrect number of arguents passed to update.. won't update.. ${arguments[0]}`;
    console.error(err);
    throw new Error(err);
  }
}
...
CrontabManager.prototype.deleteJob = function (key) {
  try {
    ...
    const error = `error in trying to stop job: ${key}: ${err}`;
    console.error(error);
...
CrontabManager.prototype.start = function (key) {
  try {
    ...
    const error = `couldn't start job: ${key}: ${err}`;
    console.error(error);
    throw new Error(error);
  }
}
...
CrontabManager.prototype.stop = function (key) {
  try {
    ...
    const error = `couldn't stop job: ${key}: ${err}`;
    console.error(error);
    throw new Error(error);
  }
}
...
function updateTab(key, cronstring) {
  try {
    ...
    const err = `Error updating tab: ${key} - ${tabErr.message}`
    console.error(err);
    throw new Error(err);
  }
}
...
function updateTask(key, task) {
  try {
    ...
    if (!(task instanceof Function)) {
      const err= `Can't update with something that is not a function: ${typeof(task)}`
      console.error(err);
      throw new Error(err);
    }
    this.jobs[key] = new CronJob(this.jobs[key].cronTime.source, task, this.jobs[key].onComplete, running, this.jobs[key].zone)
  } catch (tabErr) {
    const err = `Error updating task: ${key} - ${tabErr.message}`;
    console.error(err);
    throw new Error(err);
  }
}
